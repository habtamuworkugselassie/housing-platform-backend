...
@Repository
public interface SponsorshipApplicationRepository extends JpaRepository<SponsorshipApplication, UUID> {
    ...
    @Query("SELECT sa FROM SponsorshipApplication sa WHERE sa.organization.id = :organizationId " +
           "AND sa.status = 'APPROVED' " +
           "AND sa.startDate <= :now AND sa.endDate >= :now " +
           "ORDER BY sa.sponsorship.type DESC, sa.startDate DESC")
    Optional<SponsorshipApplication> findActiveApplication(@Param("organizationId") UUID organizationId, 
                                                           @Param("now") LocalDateTime now);
    
    @Query("SELECT sa FROM SponsorshipApplication sa WHERE sa.status = 'APPROVED' " +
           "AND sa.startDate <= :now AND sa.endDate >= :now " +
           "ORDER BY sa.sponsorship.type DESC, sa.startDate DESC")
