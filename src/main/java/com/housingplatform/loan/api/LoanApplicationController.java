...
public class LoanApplicationController {
    ...
    public ResponseEntity<LoanApplicationResponse> createLoanApplication(
        ...
        LoanApplicationResponse created = loanApplicationService.createLoanApplication(userId, applicationRequest);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }
    
    @GetMapping("/{id}")
    @Operation(summary = "Get loan application", description = "Retrieve detailed information about a loan application")
    ...
    @AuthPolicyScope(AuthPolicyScope.Policy.BUYER_SECURED)
    @Operation(summary = "Get my loan applications", description = "Retrieve all loan applications for the current buyer")
    public ResponseEntity<List<LoanApplicationResponse>> getMyLoanApplications() {
        UUID userId = com.housingplatform.shared.security.UserContext.getCurrentUserId();
        List<LoanApplicationResponse> applications = loanApplicationService.getLoanApplicationsByBuyerId(userId);
        return ResponseEntity.ok(applications);
    }
    
    @GetMapping("/buyers/{buyerId}")
    @Operation(summary = "Get buyer's loan applications", description = "Retrieve all loan applications for a buyer")
    public ResponseEntity<List<LoanApplicationResponse>> getBuyerLoanApplications(@PathVariable UUID buyerId) {
        List<LoanApplicationResponse> applications = loanApplicationService.getLoanApplicationsByBuyerId(buyerId);
        return ResponseEntity.ok(applications);
    }
    
    @GetMapping("/my-bank-applications")
    @AuthPolicyScope(AuthPolicyScope.Policy.BANKER_SECURED)
    @Operation(summary = "Get my bank's loan applications", description = "Retrieve all loan applications for the current banker's bank")
    public ResponseEntity<List<LoanApplicationResponse>> getMyBankLoanApplications() {
        ...
        List<LoanApplicationResponse> applications = loanApplicationService.getLoanApplicationsByBankId(bankId);
        return ResponseEntity.ok(applications);
    }
    
    @GetMapping("/banks/{bankId}")
    @Operation(summary = "Get bank's loan applications", description = "Retrieve all loan applications for a bank")
    public ResponseEntity<List<LoanApplicationResponse>> getBankLoanApplications(@PathVariable UUID bankId) {
        List<LoanApplicationResponse> applications = loanApplicationService.getLoanApplicationsByBankId(bankId);
        return ResponseEntity.ok(applications);
    }
    ...
    @Operation(summary = "Move to review", description = "Move a loan application to UNDER_REVIEW status (banker only)")
    ...
    @Operation(summary = "Approve loan application", description = "Approve a loan application (banker only)")
    public ResponseEntity<LoanApplicationResponse> approveLoanApplication(
        ...
        UUID bankId = organizationService.getMyBank().getId();
        LoanApplicationResponse updated = loanApplicationService.approveLoanApplication(bankId, id, approvalRequest);
        return ResponseEntity.ok(updated);
    }
    ...
    @Operation(summary = "Reject loan application", description = "Reject a loan application (banker only)")
    public ResponseEntity<LoanApplicationResponse> rejectLoanApplication(
            @PathVariable UUID id,
            @Valid @RequestBody LoanRejectionRequest rejectionRequest) {
        UUID bankId = organizationService.getMyBank().getId();
        LoanApplicationResponse updated = loanApplicationService.rejectLoanApplication(bankId, id, rejectionRequest);
        return ResponseEntity.ok(updated);
    }
}
