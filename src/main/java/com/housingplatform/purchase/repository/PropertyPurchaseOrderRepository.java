...
public interface PropertyPurchaseOrderRepository
    extends JpaRepository<PropertyPurchaseOrder, UUID>,
        JpaSpecificationExecutor<PropertyPurchaseOrder> {

  /** One row per status that has at least one order; used for the admin overview. */
  ...
  @Query(
      "SELECT o.status AS status, COUNT(o) AS count FROM PropertyPurchaseOrder o GROUP BY o.status")
