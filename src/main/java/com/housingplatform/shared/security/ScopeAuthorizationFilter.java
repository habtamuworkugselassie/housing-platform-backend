...
public class ScopeAuthorizationFilter extends OncePerRequestFilter {
    ...
    private static final Set<Class<? extends Annotation>> ANNOTATION_SET = Stream.of(
            AuthPolicyScope.class, 
            AuthActionScope.class
    ...
    protected void doFilterInternal(
        ...
        // Skip public endpoints (but allow /api/v1/auth/logout to go through as it requires authentication)
        ...
        try {
            ...
            Set<Annotation> methodAuthScopeAnnotations = AnnotatedElementUtils.getAllMergedAnnotations(
                    method.getMethod(), ANNOTATION_SET);
            
            Set<Annotation> classAuthScopeAnnotations = AnnotatedElementUtils.getAllMergedAnnotations(
            ...
            for (Annotation methodAnnotation : methodAuthScopeAnnotations) {
                authScopeAnnotations.removeIf(classAnnotation -> 
            ...
            throw new ServletException("Error resolving endpoint scope annotations: " + e.getMessage(), e);
        }
        ...
        if (authentication != null && 
            ...
            Set<String> tokenScopes = token.getScopes().stream()
                    .map(String::toLowerCase)
                    .collect(Collectors.toSet());
            
            Set<String> requiredScopesLower = requiredScopes.stream()
                    .map(String::toLowerCase)
                    .collect(Collectors.toSet());
            
            // Check if token has at least one of the required scopes
            boolean hasRequiredScope = requiredScopesLower.isEmpty() || 
                    requiredScopesLower.stream().anyMatch(tokenScopes::contains) ||
                    token.hasScope("admin"); // Admin has access to everything
            
            if (!hasRequiredScope) {
                log.warn("Access denied for path: {} - Required scopes: {}, Token scopes: {}", 
                        path, requiredScopes, tokenScopes);
                throw new AccessDeniedException(
                        String.format("Access denied. Required scopes: %s, but token has: %s", 
