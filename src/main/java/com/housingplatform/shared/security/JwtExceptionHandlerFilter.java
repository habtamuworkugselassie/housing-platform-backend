...
/**
 * Filter to catch JWT exceptions and return proper 401 responses
 * This runs before the BearerTokenAuthenticationFilter to handle exceptions gracefully
 */
public class JwtExceptionHandlerFilter extends OncePerRequestFilter {
    ...
    private void handleJwtException(HttpServletRequest request, HttpServletResponse response,
                                     Exception exception, Throwable cause) throws IOException {
        String path = request.getRequestURI();
        
        // Determine error message based on exception type
        String errorMessage = "JWT token is invalid.";
        boolean isExpired = cause instanceof ExpiredJwtException ||
