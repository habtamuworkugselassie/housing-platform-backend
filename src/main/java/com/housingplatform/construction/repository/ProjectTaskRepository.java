...
public interface ProjectTaskRepository extends JpaRepository<ProjectTask, UUID> {
    ...
    @Query("SELECT t FROM ProjectTask t WHERE t.phase.id = :phaseId ORDER BY t.sequence ASC, t.createdAt ASC")
    ...
    @Query("SELECT t FROM ProjectTask t WHERE t.assignedTo = :userId AND t.status != 'COMPLETED' AND t.status != 'CANCELLED'")
    List<ProjectTask> findActiveTasksByAssignedUser(@Param("userId") UUID userId);
    
    @Query("SELECT t FROM ProjectTask t WHERE t.parentTask.id = :parentTaskId ORDER BY t.sequence ASC")
    ...
    List<ProjectTask> findByPhaseIdAndStatus(@Param("phaseId") UUID phaseId, @Param("status") ProjectTask.TaskStatus status);
    
    @Query("SELECT COUNT(t) FROM ProjectTask t WHERE t.phase.id = :phaseId AND t.status = 'COMPLETED'")
