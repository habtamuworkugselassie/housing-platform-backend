...
public interface MaterialUsageRepository extends JpaRepository<MaterialUsage, UUID> {
  ...
  @Query(
      "SELECT u FROM MaterialUsage u WHERE u.projectId = :projectId AND u.usageDate BETWEEN :startDate AND :endDate")
  ...
  @Query(
      "SELECT SUM(u.quantity) FROM MaterialUsage u WHERE u.materialId = :materialId AND u.projectId = :projectId")
  java.math.BigDecimal getTotalUsageByMaterialAndProject(
      @Param("materialId") UUID materialId, @Param("projectId") UUID projectId);
