...
public interface ProjectMilestoneRepository extends JpaRepository<ProjectMilestone, UUID> {
    ...
    @Query("SELECT m FROM ProjectMilestone m WHERE m.project.id = :projectId ORDER BY m.targetDate ASC")
    ...
    List<ProjectMilestone> findByProjectIdAndStatus(@Param("projectId") UUID projectId, @Param("status") ProjectMilestone.MilestoneStatus status);
    ...
    @Query("SELECT m FROM ProjectMilestone m WHERE m.targetDate BETWEEN :startDate AND :endDate AND m.status != 'ACHIEVED' AND m.status != 'CANCELLED'")
    List<ProjectMilestone> findUpcomingMilestones(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
}
