...
public interface ConstructionProjectRepository extends JpaRepository<ConstructionProject, UUID> {
    ...
    Page<ConstructionProject> findByStatus(ConstructionProject.ProjectStatus status, Pageable pageable);
    
    @Query("SELECT p FROM ConstructionProject p WHERE p.realEstateCompanyId = :companyId AND p.status = :status")
    ...
    Page<ConstructionProject> findByProjectManagerId(@Param("managerId") UUID managerId, Pageable pageable);
}
