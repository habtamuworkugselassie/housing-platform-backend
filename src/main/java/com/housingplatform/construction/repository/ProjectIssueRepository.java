...
public interface ProjectIssueRepository extends JpaRepository<ProjectIssue, UUID> {
    ...
    List<ProjectIssue> findByProjectIdAndStatus(@Param("projectId") UUID projectId, @Param("status") ProjectIssue.IssueStatus status);
    
    @Query("SELECT i FROM ProjectIssue i WHERE i.assignedTo = :userId AND i.status != 'RESOLVED' AND i.status != 'CLOSED' AND i.status != 'CANCELLED'")
    ...
    List<ProjectIssue> findByProjectIdAndSeverity(@Param("projectId") UUID projectId, @Param("severity") ProjectIssue.IssueSeverity severity);
    
    @Query("SELECT i FROM ProjectIssue i WHERE i.project.id = :projectId AND i.type = :type")
    List<ProjectIssue> findByProjectIdAndType(@Param("projectId") UUID projectId, @Param("type") ProjectIssue.IssueType type);
}
