...
public interface MaterialInventoryRepository extends JpaRepository<MaterialInventory, UUID> {
    ...
    Optional<MaterialInventory> findByMaterialIdAndWarehouseLocation(UUID materialId, String warehouseLocation);
    ...
    @Query("SELECT i FROM MaterialInventory i WHERE i.availableQuantity <= i.minimumStockLevel AND i.status != 'OUT_OF_STOCK'")
