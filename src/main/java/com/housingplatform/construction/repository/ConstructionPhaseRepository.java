...
public interface ConstructionPhaseRepository extends JpaRepository<ConstructionPhase, UUID> {
    ...
    List<ConstructionPhase> findByProjectIdAndStatus(UUID projectId, ConstructionPhase.PhaseStatus status);
    
    @Query("SELECT p FROM ConstructionPhase p WHERE p.project.id = :projectId ORDER BY p.sequence ASC")
