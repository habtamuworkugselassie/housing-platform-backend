...
public interface ProjectTeamMemberRepository extends JpaRepository<ProjectTeamMember, UUID> {
    ...
    @Query("SELECT tm FROM ProjectTeamMember tm WHERE tm.project.id = :projectId AND tm.status = 'ACTIVE'")
    ...
    @Query("SELECT tm FROM ProjectTeamMember tm WHERE tm.project.id = :projectId AND tm.userId = :userId")
    Optional<ProjectTeamMember> findByProjectIdAndUserId(@Param("projectId") UUID projectId, @Param("userId") UUID userId);
    
    @Query("SELECT tm FROM ProjectTeamMember tm WHERE tm.phase.id = :phaseId AND tm.status = 'ACTIVE'")
    ...
    List<ProjectTeamMember> findByProjectIdAndRole(@Param("projectId") UUID projectId, @Param("role") ProjectTeamMember.TeamRole role);
}
