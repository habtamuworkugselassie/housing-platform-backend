...
public class MaterialUsageController {
    ...
    @Operation(summary = "Record material usage", description = "Record material usage for a project or phase")
    public ResponseEntity<MaterialUsageResponse> recordUsage(@Valid @RequestBody MaterialUsageRequest request) {
    ...
    @Operation(summary = "Get usage by project", description = "Retrieve all material usage records for a project")
    ...
    @Operation(summary = "Get all usage by project", description = "Retrieve all material usage records for a project (no pagination)")
    public ResponseEntity<List<MaterialUsageResponse>> getAllUsageByProject(@PathVariable UUID projectId) {
        List<MaterialUsageResponse> usage = usageService.getUsageByProject(projectId);
        return ResponseEntity.ok(usage);
    }
    
    @GetMapping("/phase/{phaseId}")
    @Operation(summary = "Get usage by phase", description = "Retrieve all material usage records for a construction phase")
    ...
    @Operation(summary = "Get usage by material", description = "Retrieve all material usage records for a specific material")
    public ResponseEntity<List<MaterialUsageResponse>> getUsageByMaterial(@PathVariable UUID materialId) {
    ...
    @Operation(summary = "Get total usage", description = "Get total quantity used for a material in a project")
    public ResponseEntity<java.math.BigDecimal> getTotalUsage(
            @PathVariable UUID projectId,
            @PathVariable UUID materialId) {
        java.math.BigDecimal total = usageService.getTotalUsageByMaterialAndProject(materialId, projectId);
        return ResponseEntity.ok(total);
    }
    
    @DeleteMapping("/{id}")
    @AuthActionScope("construction.usage.delete")
    @Operation(summary = "Delete usage record", description = "Delete a material usage record (reverses inventory and cost adjustments)")
