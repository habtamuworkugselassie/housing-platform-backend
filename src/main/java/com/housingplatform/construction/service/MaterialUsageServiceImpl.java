...
public class MaterialUsageServiceImpl implements MaterialUsageService {
    ...
    public MaterialUsageResponse recordUsage(UUID userId, MaterialUsageRequest request) {
        ...
        ConstructionProject project = projectRepository.findById(request.getProjectId())
                .orElseThrow(() -> new ResourceNotFoundException("Construction Project", request.getProjectId()));
        
        // Validate phase if provided
        ConstructionPhase phase = null;
        if (request.getPhaseId() != null) {
            phase = phaseRepository.findById(request.getPhaseId())
                    .orElseThrow(() -> new ResourceNotFoundException("Construction Phase", request.getPhaseId()));
        ...
        if (request.getInventoryId() != null) {
            inventory = inventoryRepository.findById(request.getInventoryId())
                    .orElseThrow(() -> new ResourceNotFoundException("Material Inventory", request.getInventoryId()));
            
            // Check if enough quantity is available
            if (inventory.getAvailableQuantity().compareTo(request.getQuantity()) < 0) {
                throw new BusinessException("Insufficient inventory. Available: " + inventory.getAvailableQuantity() + 
                        ", Requested: " + request.getQuantity());
            }
            
            // Adjust inventory
            inventory.setQuantity(inventory.getQuantity().subtract(request.getQuantity()));
            inventory.setAvailableQuantity(inventory.getAvailableQuantity().subtract(request.getQuantity()));
        ...
        if (phase != null && usage.getTotalCost() != null) {
            BigDecimal currentCost = phase.getActualCost() != null ? phase.getActualCost() : BigDecimal.ZERO;
        ...
        if (usage.getTotalCost() != null) {
            BigDecimal currentCost = project.getTotalCost() != null ? project.getTotalCost() : BigDecimal.ZERO;
    ...
    public MaterialUsageResponse getUsageById(UUID id) {
        MaterialUsage usage = usageRepository.findById(id)
    ...
    public List<MaterialUsageResponse> getUsageByProject(UUID projectId) {
        return usageRepository.findByProjectId(projectId)
                .stream()
    ...
    public List<MaterialUsageResponse> getUsageByPhase(UUID phaseId) {
        return usageRepository.findByPhaseId(phaseId)
                .stream()
                .map(this::enrichUsageResponse)
                .collect(Collectors.toList());
    }
    ...
    public List<MaterialUsageResponse> getUsageByMaterial(UUID materialId) {
        return usageRepository.findByMaterialId(materialId)
                .stream()
                .map(this::enrichUsageResponse)
                .collect(Collectors.toList());
    }
    ...
    public Page<MaterialUsageResponse> getUsageByProject(UUID projectId, Pageable pageable) {
        return usageRepository.findByProjectId(projectId, pageable)
                .map(this::enrichUsageResponse);
    }
    ...
    public void deleteUsage(UUID id) {
        ...
        if (usage.getInventoryId() != null) {
            MaterialInventory inventory = inventoryRepository.findById(usage.getInventoryId())
                    .orElseThrow(() -> new ResourceNotFoundException("Material Inventory", usage.getInventoryId()));
        ...
        if (usage.getTotalCost() != null) {
            ConstructionProject project = projectRepository.findById(usage.getProjectId())
                    .orElseThrow(() -> new ResourceNotFoundException("Construction Project", usage.getProjectId()));
            project.setTotalCost(project.getTotalCost().subtract(usage.getTotalCost()));
            projectRepository.save(project);
            
            if (usage.getPhaseId() != null) {
                ConstructionPhase phase = phaseRepository.findById(usage.getPhaseId())
                        .orElseThrow(() -> new ResourceNotFoundException("Construction Phase", usage.getPhaseId()));
    ...
    private MaterialUsageResponse enrichUsageResponse(MaterialUsage usage) {
        ...
        // Enrich project name
        projectRepository.findById(usage.getProjectId())
                .ifPresent(project -> response.setProjectName(project.getName()));
        
        // Enrich phase name
        if (usage.getPhaseId() != null) {
            phaseRepository.findById(usage.getPhaseId())
        ...
        if (usage.getOrderId() != null) {
            orderRepository.findById(usage.getOrderId())
