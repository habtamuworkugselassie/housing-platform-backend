...
public class MaterialInventoryServiceImpl implements MaterialInventoryService {
    ...
    public MaterialInventoryResponse createInventory(MaterialInventoryRequest request) {
        ...
        if (request.getProjectId() != null) {
            existing = inventoryRepository.findByMaterialIdAndProjectId(
                    request.getMaterialId(), request.getProjectId()).orElse(null);
        } else {
            existing = inventoryRepository.findByMaterialIdAndWarehouseLocation(
                    request.getMaterialId(), request.getWarehouseLocation()).orElse(null);
        }
        
        if (existing != null) {
            throw new BusinessException("Inventory already exists for this material and location/project");
        }
    ...
    @Override
    @Transactional(readOnly = true)
    public MaterialInventoryResponse getInventoryByMaterialAndProject(UUID materialId, UUID projectId) {
        MaterialInventory inventory = inventoryRepository.findByMaterialIdAndProjectId(materialId, projectId)
                .orElseThrow(() -> new ResourceNotFoundException("Material Inventory", "materialId: " + materialId + ", projectId: " + projectId));
        return enrichInventoryResponse(inventory);
    }
    ...
    public List<MaterialInventoryResponse> getInventoryByProject(UUID projectId) {
        return inventoryRepository.findByProjectId(projectId)
                .stream()
                .map(this::enrichInventoryResponse)
                .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<MaterialInventoryResponse> getInventoryByStatus(MaterialInventory.InventoryStatus status, Pageable pageable) {
    ...
    public MaterialInventoryResponse updateInventory(UUID id, MaterialInventoryRequest request) {
        ...
        inventory.setAvailableQuantity(inventory.getQuantity().subtract(inventory.getReservedQuantity()));
    ...
    @Override
    public MaterialInventoryResponse adjustInventory(UUID id, BigDecimal quantityChange, String reason) {
        MaterialInventory inventory = inventoryRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Material Inventory", id));
