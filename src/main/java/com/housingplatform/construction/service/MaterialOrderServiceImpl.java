...
public class MaterialOrderServiceImpl implements MaterialOrderService {
    ...
    public MaterialOrderResponse createOrder(UUID userId, MaterialOrderRequest request) {
        ...
        organizationRepository.findById(request.getSupplierId())
                .filter(org -> org.getType() == com.housingplatform.identity.domain.Organization.OrganizationType.SUPPLIER)
        ...
        // Create order items
        List<MaterialOrderItem> items = request.getItems().stream()
                .map(itemRequest -> {
    ...
    public MaterialOrderResponse getOrderByOrderNumber(String orderNumber) {
        MaterialOrder order = orderRepository.findByOrderNumber(orderNumber)
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<MaterialOrderResponse> getOrdersBySupplier(UUID supplierId, MaterialOrder.OrderStatus status, Pageable pageable) {
    ...
    @Override
    public MaterialOrderResponse receiveOrderItems(UUID orderId, UUID itemId, BigDecimal receivedQuantity) {
        MaterialOrder order = orderRepository.findById(orderId)
                .orElseThrow(() -> new ResourceNotFoundException("Material Order", orderId));
        
        MaterialOrderItem item = orderItemRepository.findById(itemId)
    ...
    private MaterialOrderResponse enrichOrderResponse(MaterialOrder order) {
        ...
        organizationRepository.findById(order.getSupplierId())
                .ifPresent(org -> response.setSupplierName(org.getName()));
        
        // Enrich order items
        List<MaterialOrderItem> items = orderItemRepository.findByOrderIdOrderBySequenceAsc(order.getId());
        List<MaterialOrderItemResponse> itemResponses = items.stream()
