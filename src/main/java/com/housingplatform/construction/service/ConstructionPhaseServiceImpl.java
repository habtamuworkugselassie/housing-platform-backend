...
public class ConstructionPhaseServiceImpl implements ConstructionPhaseService {
    ...
    @Override
    public ConstructionPhaseResponse updatePhase(UUID projectId, UUID phaseId, ConstructionPhaseRequest request) {
    ...
    @Override
    public ConstructionPhaseResponse updatePhaseStatus(UUID phaseId, ConstructionPhase.PhaseStatus status) {
        ConstructionPhase phase = phaseRepository.findById(phaseId)
                .orElseThrow(() -> new ResourceNotFoundException("Construction Phase", phaseId));
        ...
        } else if (status == ConstructionPhase.PhaseStatus.IN_PROGRESS && phase.getStartDate() == null) {
    ...
    @Override
    public ConstructionPhaseResponse updatePhaseCompletion(UUID phaseId, Integer completionPercentage) {
        ...
        } else if (completionPercentage > 0 && phase.getStatus() == ConstructionPhase.PhaseStatus.NOT_STARTED) {
    ...
    public void reorderPhases(UUID projectId, List<UUID> phaseIdsInOrder) {
        ...
        for (int i = 0; i < phaseIdsInOrder.size(); i++) {
            ...
            ConstructionPhase phase = phases.stream()
