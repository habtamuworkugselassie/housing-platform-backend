...
public class FinancingOfferServiceImpl implements FinancingOfferService {
    ...
    public FinancingOfferResponse getFinancingOfferById(UUID id) {
        FinancingOffer offer = financingOfferRepository.findById(id)
    ...
    public List<FinancingOfferResponse> getFinancingOffersByBankId(UUID bankId) {
        return financingOfferRepository.findByBankId(bankId)
                .stream()
    ...
    public List<FinancingOfferResponse> getFinancingOffersByPropertyId(UUID propertyId) {
        return financingOfferRepository.findByPropertyIdAndStatus(propertyId, FinancingOffer.FinancingOfferStatus.ACTIVE)
                .stream()
                .map(financingOfferMapper::toResponse)
                .map(this::enrichWithCreditProductDetails)
                .collect(Collectors.toList());
    ...
    public List<FinancingOfferResponse> getFinancingOffersByProductId(UUID productId) {
        return financingOfferRepository.findByCreditProductIdAndStatus(productId, FinancingOffer.FinancingOfferStatus.ACTIVE)
                .stream()
                .map(financingOfferMapper::toResponse)
                .map(this::enrichWithCreditProductDetails)
                .collect(Collectors.toList());
    ...
    public List<FinancingOfferResponse> getFinancingOffersByBuildingId(UUID buildingId) {
        return financingOfferRepository.findByBuildingIdAndStatus(buildingId, FinancingOffer.FinancingOfferStatus.ACTIVE)
                .stream()
                .map(financingOfferMapper::toResponse)
                .map(this::enrichWithCreditProductDetails)
                .collect(Collectors.toList());
    ...
    @Override
    public FinancingOfferResponse linkCreditProductToProperty(UUID bankId, UUID creditProductId, UUID propertyId) {
        // Check if offer already exists
        List<FinancingOffer> existing = financingOfferRepository.findByPropertyId(propertyId)
                .stream()
                .filter(offer -> offer.getCreditProductId().equals(creditProductId) && offer.getBankId().equals(bankId))
                .collect(Collectors.toList());
    ...
    @Override
    public FinancingOfferResponse linkCreditProductToBuilding(UUID bankId, UUID creditProductId, UUID buildingId) {
        // Check if offer already exists
        List<FinancingOffer> existing = financingOfferRepository.findByBuildingId(buildingId)
                .stream()
                .filter(offer -> offer.getCreditProductId().equals(creditProductId) && offer.getBankId().equals(bankId))
                .collect(Collectors.toList());
    ...
    @Override
    public FinancingOfferResponse updateFinancingOffer(UUID bankId, UUID offerId, FinancingOfferRequest request) {
    ...
    private FinancingOfferResponse enrichWithCreditProductDetails(FinancingOfferResponse response) {
        if (response.getCreditProductId() != null) {
            creditProductRepository.findById(response.getCreditProductId())
                    .ifPresent(product -> {
