...
public class CreditProductController {
    ...
    @Operation(summary = "List credit products", description = "Retrieve all credit products for a bank. Public access.")
    ...
    @Operation(summary = "Create credit product", description = "Create a new credit product for a bank")
    public ResponseEntity<CreditProductResponse> createCreditProduct(
            @PathVariable UUID bankId,
            @Valid @RequestBody CreditProductRequest productRequest) {
        CreditProductResponse created = creditProductService.createCreditProduct(bankId, productRequest);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }
    
    @GetMapping("/{productId}")
    @AuthPolicyScope(AuthPolicyScope.Policy.UNSECURED)
    @Operation(summary = "Get credit product", description = "Retrieve a specific credit product. Public access.")
    ...
    public ResponseEntity<CreditProductResponse> updateCreditProduct(
            ...
            @PathVariable UUID productId,
            @Valid @RequestBody CreditProductRequest productRequest) {
        CreditProductResponse updated = creditProductService.updateCreditProduct(bankId, productId, productRequest);
    ...
    @AuthPolicyScope(AuthPolicyScope.Policy.UNSECURED)
    @Operation(summary = "Get financing offers", description = "Retrieve all financing offers for a credit product. Public access.")
    public ResponseEntity<List<FinancingOfferResponse>> getFinancingOffers(
            @PathVariable UUID bankId,
            @PathVariable UUID productId) {
        List<FinancingOfferResponse> offers = financingOfferService.getFinancingOffersByProductId(productId);
