...
public class FinancingOfferController {
  ...
  @Operation(
      summary = "Create financing offer",
      description = "Create a new financing offer linked to a property or project")
  public ResponseEntity<FinancingOfferResponse> createFinancingOffer(
      @PathVariable UUID bankId, @Valid @RequestBody FinancingOfferRequest offerRequest) {
    FinancingOfferResponse created =
        financingOfferService.createFinancingOffer(bankId, offerRequest);
    return ResponseEntity.status(HttpStatus.CREATED).body(created);
  }

  @GetMapping
  @Operation(
      summary = "List financing offers",
      description = "Retrieve all financing offers for a bank")
  ...
  public ResponseEntity<FinancingOfferResponse> getFinancingOffer(
      @PathVariable UUID bankId, @PathVariable UUID offerId) {
  ...
  @Operation(
      summary = "Get offers for property",
      description = "Retrieve all financing offers for a specific property")
  public ResponseEntity<List<FinancingOfferResponse>> getOffersForProperty(
      @PathVariable UUID bankId, @PathVariable UUID propertyId) {
    List<FinancingOfferResponse> offers =
        financingOfferService.getFinancingOffersByPropertyId(propertyId);
    return ResponseEntity.ok(offers);
  }
  ...
  public ResponseEntity<FinancingOfferResponse> updateFinancingOffer(
    ...
    FinancingOfferResponse updated =
        financingOfferService.updateFinancingOffer(bankId, offerId, offerRequest);
  ...
  @Operation(
      ...
      description =
          "Quickly link a credit product to a property, creating an active financing offer")
  public ResponseEntity<FinancingOfferResponse> linkToProperty(
      @PathVariable UUID bankId,
      @RequestParam UUID creditProductId,
      @RequestParam UUID propertyId) {
    FinancingOfferResponse offer =
        financingOfferService.linkCreditProductToProperty(bankId, creditProductId, propertyId);
  ...
  @AuthActionScope("financing-offers.create")
  @Operation(
      summary = "Link credit product to building",
      description =
          "Quickly link a credit product to a building, creating an active financing offer")
  public ResponseEntity<FinancingOfferResponse> linkToBuilding(
      @PathVariable UUID bankId,
      @RequestParam UUID creditProductId,
      @RequestParam UUID buildingId) {
    FinancingOfferResponse offer =
        financingOfferService.linkCreditProductToBuilding(bankId, creditProductId, buildingId);
    return ResponseEntity.status(HttpStatus.CREATED).body(offer);
  }

  @GetMapping("/buildings/{buildingId}")
  @Operation(
      summary = "Get offers for building",
      description = "Retrieve all financing offers for a specific building")
  public ResponseEntity<List<FinancingOfferResponse>> getOffersForBuilding(
      @PathVariable UUID bankId, @PathVariable UUID buildingId) {
    List<FinancingOfferResponse> offers =
        financingOfferService.getFinancingOffersByBuildingId(buildingId);
    return ResponseEntity.ok(offers);
  }
}
