...
class AuthenticationServiceTest {
    ...
    void testLogin_Success() {
        ...
        when(passwordEncoder.matches("password123", testUser.getPasswordHash()))
                .thenReturn(true);
        when(jwtTokenProvider.generateToken(any(UUID.class), anyString(), any(), any()))
        ...
        verify(jwtTokenProvider).generateToken(any(UUID.class), anyString(), any(), any());
    ...
    void testLogin_WithPhoneNumber() {
        ...
        when(userRepository.findByEmail("+251911111111"))
                .thenReturn(Optional.empty());
        when(userRepository.findByPhoneNumber("+251911111111"))
                .thenReturn(Optional.of(testUser));
        when(passwordEncoder.matches("password123", testUser.getPasswordHash()))
                .thenReturn(true);
        when(jwtTokenProvider.generateToken(any(UUID.class), anyString(), any(), any()))
    ...
    void testLogin_InvalidCredentials_UserNotFound() {
        ...
        when(userRepository.findByEmail("nonexistent@example.com"))
                .thenReturn(Optional.empty());
        when(userRepository.findByPhoneNumber("nonexistent@example.com"))
                .thenReturn(Optional.empty());
    ...
    void testRegister_Success() {
        ...
        when(userRepository.findByEmail("newuser@example.com"))
                .thenReturn(Optional.empty());
        when(userRepository.findByPhoneNumber("+251922222222"))
                .thenReturn(Optional.empty());
    ...
    void testRegister_AdminRoleNotAllowed() {
        ...
        assertEquals("Admin role cannot be self-assigned. Please contact system administrator.", exception.getMessage());
        verify(userRepository, never()).save(any(User.class));
    }
}
